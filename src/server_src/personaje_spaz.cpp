...
Spaz::Spaz(uint32_t id_jugador) : Personaje(id_jugador){
    tiempo_especial = -ENFRIAMIENTO_SPAZ;
}

uint8_t Spaz::obtener_personaje(){
    return SPAZ;
}

void Spaz::usar_habilidad(std::chrono::duration<double> tiempo_transcurrido){
    if(!saltando && !usando_especial && (tiempo_transcurrido.count() - tiempo_especial) >= ENFRIAMIENTO_JAZZ){
        esta_quieto = false;
