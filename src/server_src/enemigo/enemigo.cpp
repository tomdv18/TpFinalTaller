...
uint32_t Enemigo::obtener_alto() { return alto; }

uint32_t Enemigo::obtener_ancho() { return ancho; }

bool Enemigo::mirando_izquierda() { return direccion_mirando == IZQUIERDA; }
    ...
    switch (direccion_mirando) {
        case DERECHA:
            if (pasos_patrullando >= PASOS_POR_DIRECCION) {
                ...
                //<< std::endl;
                // std::cout << "CAMBIO DE DIRECCION PATRULLAJE DE DERECHA A IZQUIERDA" <<
                // std::endl;
            } else {
                mover_derecha();
                pasos_patrullando++;
        ...
        case IZQUIERDA:
            if (pasos_patrullando >= PASOS_POR_DIRECCION) {
                ...
                // std::cout << "POSICION DEL ENEMIGO (" << posicion_x << ", " << posicion_y << ")"
                //        << std::endl;
                // std::cout << "CAMBIO DE DIRECCION PATRULLAJE DE IZQUIERDA A DERECHA" <<
                // std::endl;
            } else {
                mover_izquierda();
                pasos_patrullando++;
...
void Enemigo::actualizar_posicion(std::chrono::duration<double> tiempo_transcurrido) {
    ...
    if (tiempo_transcurrido.count() - tiempo_muerte >= tiempo_reaparicion) {
        vida = CONFIG.obtenerVidaDefaultEnemigo();  // POTENCIAL BUG SI NO SE IMPLEMENTA EN LAS
                                                    // CLASES HIJAS
        vivo = true;
    }
