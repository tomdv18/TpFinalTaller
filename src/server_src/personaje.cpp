...
void Personaje::dejar_disparar() { esta_disparando = false; }
...
void Personaje::correr_rapido() { corriendo = saltando ? false : true; }

void Personaje::correr() { corriendo = false; }

void Personaje::recibir_golpe(uint8_t golpe) { this->vida -= golpe; }
...
void Personaje::actualizar_posicion(std::chrono::duration<double> tiempo_transcurrido,
    ...
    Rectangulo rect_personaje = {posicion_x + velocidad_x, posicion_y, PERSONAJE_WIDTH,
                                 PERSONAJE_HEIGHT};

    Rectangulo rect_personaje_arriba = {posicion_x, posicion_y + velocidad_y, PERSONAJE_WIDTH,
                                        PERSONAJE_HEIGHT};
