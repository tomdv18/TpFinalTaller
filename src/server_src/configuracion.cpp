...
static const char configPath[] = CONFIG_PATH;
...
Configuracion::Configuracion(const YAML::Node& config) {
    ...
    std::map<std::string, uint8_t> codigo_balas = {{"bala_normal", BALA_NORMAL},
    ...
    std::map<std::string, uint8_t> codigo_objetos = {
            {"zanahoria", ZANAHORIA}, {"gema", GEMA}, {"moneda", MONEDA}};
    ...
    for (const auto& entry: config["objeto"]) {
        ...
        if (codigo_objetos.find(codigo_objeto_str) != codigo_objetos.end()) {
            ...
            config_objeto.vida_restaurada = 0;   // Default value
            config_objeto.puntos_otorgados = 0;  // Default value
